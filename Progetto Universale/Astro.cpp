...
//#include <windows.h> // commentare per Linux os - Modificato da SIMONE VERSIENTI
//#include <unistd.h> modificato da SIMONE VERSIENTI
#include <algorithm> //libreria utilizzata per trasformare il testo (TOUPPER) o (TOLOWER)
#include "Biglietto.h"
#include "Prodotto.h"
#include "Cucina.h"// modifiche Costantini Andrea
#include <random> //modifiche Bottiglione Margareth
#include <chrono> // modificato da SIMONE VERSIENTI
#include "Capacita.h"
...
void Astro::init_specifiche()
{
    ...
    vocabolario.inserisci("consegna", 80);
    vocabolario.inserisci("documenti", 62); //Cambiato il codice in quanto la parola documento era stata già aggiunta con un altro codice
    ...
    //FIXME RIMUOVI vocabolario.inserisci("ritira", 81); //Modifica PMF(ufficio)
    ...
    vocabolario.inserisci("spegni", 223);
    ...
    vocabolario.inserisci("suona", 222); //per utilizzare il verbo suonare
    ...
    vocabolario.inserisci("10", 91);  //Modifica Lentini Giovanni per risolvere Bug portafoglio
...
// inizio modifiche CHIARAPPA ROSA
    ...
    vocabolario.inserisci("ruba", 50);
    vocabolario.inserisci("rubati", 200);

//INIZIO MODIFICHE FLAVIO COLLOCOLA
    vocabolario.inserisci("coupon", 44);
//FINE MODIFHCE FLAVIO COLLOCOLA

//INIZIO MODIFICHE SPECCHIA ROBERTO
    vocabolario.inserisci("ritorna", 72);
    azioni.inserisci(720000, 9); // torna indietro di un luogo
//FINE MODIFICHE SPECCHIA ROBERTO

//INIZIO MODIFICHE LEANDRA PALEMBURGI
    vocabolario.inserisci("resoconto", 12);
    azioni.inserisci(120000, 12); //azione resoconto
//FINE MODIFICHE LEANDRA PALEMBURGI
...
//inizio modifiche Baso Matteo
    vocabolario.inserisci("oggetti",165);
    azioni.inserisci(1650000,165);
//fine modifiche Baso Matteo

//inizio modifiche Bottiglione Margareth
    vocabolario.inserisci("droide", 201);
    vocabolario.inserisci("utilizza", 202);
    azioni.inserisci(4802020201, 157);
//fine modifiche Bottiglione Margareth

//inizio modifiche Costantini Andrea
    vocabolario.inserisci("mela",677);
    vocabolario.inserisci("barretta_energetica",678);
    vocabolario.inserisci("bimby",680);
    vocabolario.inserisci("tavolo",681);
    vocabolario.inserisci("frigorifero",682);
    vocabolario.inserisci("televisione",683);
//fine modifiche Costantini Andrea

    //inizio modifiche Gravina Antonio
    vocabolario.inserisci("foglio", 72);
    vocabolario.inserisci("poster", 166);
    vocabolario.inserisci("vagabondo", 167);
    ...
    azioni.inserisci(180056, -46); //Modifica PMF (agenda)comando cancella agenda

    //inizio modifica Petronelli Angelo
    azioni.inserisci(1900, 8);
    //fine modifica Petronelli Angelo

    azioni.inserisci(190056, -47); // Modifica PMF (agenda) comando scrivi agenda

    azioni.inserisci(200050, -2);
    azioni.inserisci(200051, -2);
    azioni.inserisci(200052, -2);
...
//    azioni.inserisci(720000, 9); //modifica
    ...
    azioni.inserisci(1400300031,51); // parla mercante
    ...
    azioni.inserisci(1500300036,55); // parla impiegato
    ...
    //INIZIO MODIFICHE FLAVIO COLLOCOLA
    azioni.inserisci(1200800062, 140); //(CONSEGNA DOCUMENTO)
    azioni.inserisci(1200420062, 141); //(RITIRA DOCUMENTO)
    //FINE MODIFICHE FLAVIO COLLOCOLA
    //Modifica PMF: fin qui.
    ...
    //INIZIO MODIFICHE FLAVIO COLLOCOLA
    azioni.inserisci(430000, 78); //mostra il contenuto della tasca
    //FINE MODIFICHE FLAVIO COLLOCOLA
    ...
    azioni.inserisci(2502230080, 90);//"Spegni proiettore"
    //FINE modifiche Moschetti
    ...
    azioni.inserisci(2800300073, 100); //parla con il preside
    azioni.inserisci(2900300073, 101); //parla con la maestra Clara
    azioni.inserisci(3000300073, 102); //parla con la maestra Mara
    ...
    azioni.inserisci(3200300090, 108); //Parla al microfono dell'Auditorium
    ...
    azioni.inserisci(3202220088, 111);  //Usa gli strumenti musicali
    ...
    azioni.inserisci(4100300073, 139); //parla meccanico
    //FINE modifiche PALAGIANO MARCELLO
    //inizio modifiche Giovanni Lentini
...
//inizio modifiche Costantini Andrea
    azioni.inserisci(4900980678,159); // mangia barretta
    azioni.inserisci(4900980046,160); // mangia pizza
    azioni.inserisci(4900220682,161); // apri frigorifero
    azioni.inserisci(4900100683,162); // guarda televisione
    azioni.inserisci(4900800680,163); // preparazione cibo
    azioni.inserisci(980677,164); // mangia mela (modificato da Gravina Antonio)
//fine modifiche Costantini Andrea

    //inizio modifiche Gravina Antonio
    azioni.inserisci(430026,166); // elenca capacità
    azioni.inserisci(180026,167); // elimina capacià
    azioni.inserisci(1200100083,168); // guarda motorino
    azioni.inserisci(600250072,169); // leggi foglio (per giocare all'impiccato)
    azioni.inserisci(600100166,170); // guarda poster (per ottenere capacità)
    azioni.inserisci(600250166,170); // leggi poster (per ottenere capacità)
    azioni.inserisci(400300167,171); // parla vagabondo
    //fine modifiche Gravina Antonio
    ...
    oggetti.inserisci(Oggetto("5 Euro",3, 6));  //MODIFICA D-R(Pisani):Portafoglio + Carta di Credito
    oggetti.inserisci(Oggetto("10 Euro",4, 3)); //MODIFICA D-R(Pisani):Portafoglio + Carta di Credito
    oggetti.inserisci(Oggetto("50 Euro",6, 3)); //MODIFICA D-R(Pisani):Portafoglio + Carta di Credito
    oggetti.inserisci(Oggetto("1  Euro",1, 1)); //MODIFICA D-R(Pisani):Portafoglio + Carta di Credito
    oggetti.inserisci(Oggetto("2  Euro",2, 2)); //MODIFICA D-R(Pisani):Portafoglio + Carta di Credito
    oggetti.inserisci(Oggetto("10 Euro",4, 7)); //MODIFICA D-R(Pisani):Portafoglio + Carta di Credito
    oggetti.inserisci(Oggetto("20 Euro",5, 4)); //MODIFICA D-R(Pisani):Portafoglio + Carta di Credito
    oggetti.inserisci(Oggetto("1  Euro",1, 2)); //MODIFICA D-R(Pisani):Portafoglio + Carta di Credito
    oggetti.inserisci(Oggetto("2  Euro",2, 7)); //MODIFICA D-R(Pisani):Portafoglio + Carta di Credito

    oggetti.inserisci(Oggetto("un orologio", 74, -5));

    oggetti.inserisci(Oggetto("un calcolatore", 80, -12));//Modifica PMF(ufficio)
    oggetti.inserisci(Oggetto("una scrivania", 81, -12 ));//Modifica PMF(ufficio)

    oggetti.inserisci(Oggetto("una pompa di benzina",82, -13));
    oggetti.inserisci(Oggetto("un motorino",83, 12)); //28////Modifica PMF

    oggetti.inserisci(Oggetto("un distintivo spazio",87, -9999));//->50
    oggetti.inserisci(Oggetto("un distintivo pronto soccorso",88, -9999));
    oggetti.inserisci(Oggetto("un distintivo annuncio",89, -9999));
    oggetti.inserisci(Oggetto("un distintivo corridoio",90, -9999));
    oggetti.inserisci(Oggetto("un distintivo algoritmi",91, -9999));

    oggetti.inserisci(Oggetto("la rubrica", 28, -9999,"Rubrica:\n\n\tStato:\t\tPrendibile\n\tLuogo:\t\tLa rubrica e' situata nell'armadietto che si trova\n\t\t\tnella tua cabina!\n\tDescrizione:\tPuoi consultarla per poter ottenere informazioni\n\t\t\tsu alcuni personaggi coinvolti nel gioco!\n"));
    oggetti.inserisci(Oggetto("la guida", 32, 1,"Guida:\n\n\tStato:\t\tPrendibile\n\tLuogo:\t\tLa guida e' situata nella cabina di pilotaggio!\n\tDescrizione:\tPuoi consultarla per poter ottenere informazioni\n\t\t\tsu tutti gli oggetti presenti nel gioco!\n"));
    oggetti.inserisci(Oggetto("il tesserino", 31, -9999,"Tesserino:\n\n\tStato:\t\tPrendibile\n\tLuogo:\t\tIl tesserino e' situato nell'armadietto che si trova nella cabina del secondo pilota!\n\tDescrizione:\tTi serve per interagire con i comandi del reattore!\n"));

    //FINE MODIFICA DAMONE MARCO
    //INIZIO MODIFICA MICHELE ALBANO
    oggetti.inserisci(Oggetto("una tessera sanitaria",84, 6));  //Indice-->58
    oggetti.inserisci(Oggetto("un gettone",86, 12));  //Indice-->59
    oggetti.inserisci(Oggetto("un terminale",99, -19));  //Indice-->60
    //FINE MODIFICA MICHELE ALBANO

    //INIZIO MODIFICA GIOVANNI CASTELLANA
    oggetti.inserisci(Oggetto("una croce",43, 20));
    oggetti.inserisci(Oggetto("un confessionale",97, -20));
    oggetti.inserisci(Oggetto("un sacerdote",98, -20));
    //FINE MODIFICA GIOVANNI

    oggetti.inserisci(Oggetto("la Pietra Canalizzatrice",49,0));
    ...
    oggetti.inserisci(Oggetto("Fettuccine Paglia e Fieno",96,21,46,"VERDE"));
    oggetti.inserisci(Oggetto("Misto frutti di mare",95,21,38,"VERDE"));
    oggetti.inserisci(Oggetto("un tesserino ristorante verde",45,1,"VERDE"));//-_>77
    oggetti.inserisci(Oggetto("un buono pasto giallo",59,5,"GIALLO"));//-->78
    ...
    //Inizio modifiche Galeandro da progetto Bellanova
    oggetti.inserisci(Oggetto("un documento d'identita'",62,6));//80 (doc 79 da 61)
    oggetti.inserisci(Oggetto("una lettera",37,-9999));//81
    oggetti.inserisci(Oggetto("uno sportello telematico",99,-24));//Galeandro da -23 a -24
    oggetti.inserisci(Oggetto("un pacco",57,-9999));//83
    //Fine modifiche Galeandro da progetto Bellanova
    ...
    oggetti.inserisci(Oggetto("10 euro",4,-99));//modifiche Lentini Giovanni, modificato i codici oggetto per risovere il bug del portafoglio
    ...
    //INIZIO MODIFICHE FLAVIO COLLOCOLA
    //Aggiunto alla fine in modo da non cambiare le posizioni intermedie degli elementi della lista di oggetti
...
//inizio modifiche Costantini Andrea
    oggetti.inserisci(Oggetto("mela",677,49));
    oggetti.inserisci(Oggetto("barretta_energetica",678,49));
    oggetti.inserisci(Oggetto("pizza",46,49));
    oggetti.inserisci(Oggetto("bimby",680, -49));
    oggetti.inserisci(Oggetto("tavolo",681, -49));
    oggetti.inserisci(Oggetto("frigorifero",682,-49));
    oggetti.inserisci(Oggetto("televisione",683,-49));
//fine modifiche Costantini Andrea

    //inizio modifiche Gravina Antonio
    oggetti.inserisci(Oggetto("un foglio", 72, -6));
    oggetti.inserisci(Oggetto("un poster", 166, -6));
    oggetti.inserisci(Oggetto("un vagabondo", 167, -4));
    //fine modifiche Gravina Antonio
    ...
    //inizio modifiche Gravina Antonio
    capacita.inserisci("aprire", "potrebbe essere utile per aprire o chiudere una porta.", 22, 0); // codice vocabolo // codice mappa (se è negativo, l'oggetto non si può trasportare)
    capacita.inserisci("digitare", "Potrebbe essere utile in un luogo dove sono prensenti tastiere.", 92, 0);
    capacita.inserisci("leggere", "Difficilmente potrai andare avanti senza poter leggere i cartelli o i manuali!", 25, 0);
    capacita.inserisci("scrivere", "Necessaria per poter scrivere sull'agenda!", 19, 0);
    capacita.inserisci("piegarsi", "Potrebbe essere utile per vedere se c'e' qualcosa sotto qualche mobile o letto.", 58, -99);
    capacita.inserisci("premere", "Sara' sicuramente utile per poter premere qualche pulsante!", 26, -99);
    capacita.inserisci("saltare", "Potrebbe essere utile nella sala del secondo pilota.", 49, -99);
    capacita.inserisci("guidare", "Necessaria per guidare il motorino!", 48, -99);
    Astro::n_capacita_acquisite = 4; // Necessario perché save/load sono non funzionanti
    //fine modifiche Gravina Antonio
...
bool Astro::prendi_specifiche()
{
    ...
    else if (luogo_attuale == 37 || luogo_attuale == 38)
    {
        ...
        // system("cls"); // modificato da SIMONE VERSIENTI
        CLEARSCREEN();
...
bool Astro::preso_specifiche()
{
    ...
    if (og == 4 || og == 22 || og == 11 /*|| og == 26 || og == 27*/) // MODIFICA D-R(Pisani):Portafoglio + Carta di Credito
...
void Astro::save_specifiche(ofstream &file)
{
    ...
    {
        file << capacita.get_nome_capacita(indice) << '\n';
        file << capacita.get_luogo_capacita(indice) << '\n';
...
void Astro::load_specifiche(ifstream &file)
{
    ...
    while (!cap.finelista(indice))
    {
        ...
        file >> y;
        file >> x;
        capacita.set_luogo_capacita(y, x);
...

{
    ...
    if (aperto) //se il file di input è stato aperto correttamente
    {
        ...
        cout<<"Errore apertura file \"Mappa.nav\""<<endl; //altrimenti stampa messaggio di errore
}
void Astro::encomio()
{
    interfaccia.scrivi("### IL GAZZETTINO DELLA GALASSIA ###\n");
...
void Astro::azione_13()
{
    ...
    while (!cap.finelista(indice))
    {
        if (capacita.get_nome_capacita(indice) == capacita_da_controllare)
        {
            if (capacita.get_luogo_capacita(indice) == 0)
            {
                ...
                interfaccia.scrivi("Forse ti manca una capacita'...\n");
        }
        indice = cap.succlista(indice);
    }
    stringa_risposta = "forse dovevi leggere, non guardare.";
    storia_gioco.insStoria(stringa_comando, stringa_risposta);
}
//fine modifiche Gravina Antonio
...
void Astro::azione_15()
{
    ...
    if (oggetti.get_oggetto(og).get_luogo() == luogo_attuale)
    {
        ...
        stringa_risposta = "prima dovevi prenderlo.";
        storia_gioco.insStoria(stringa_comando, stringa_risposta);
    }
    else
    {
        while (!cap.finelista(indice))
        {
            if (capacita_da_controllare == capacita.get_nome_capacita(indice))
            {
                if (capacita.get_luogo_capacita(indice) == 0)
                {
                    ...
                    stringa_risposta = "hai letto il MANUALE DEL REATTORE, ma hai notato che la pagina era strappata!";
                    storia_gioco.insStoria(stringa_comando, stringa_risposta);
                }
                else
                    interfaccia.scrivi(
                        "Non hai la capacita' di leggere! Avresti dovuto pensarci prima di eliminarla! E adesso?\n");
            }
            indice = cap.succlista(indice);
        }
    }
    ...
    while (!cap.finelista(indice))
    {
        if (capacita_da_controllare == capacita.get_nome_capacita(indice))
        {
            if (capacita.get_luogo_capacita(indice) == 0)
            {
                ...
                stringa_risposta = "hai guardato l'indicatore della TEMPERATURA DEL REATTORE.";
                storia_gioco.insStoria(stringa_comando, stringa_risposta);
            }
            else
                interfaccia.scrivi("Forse ti manca una capacita'...\n");
    ...
    while (!cap.finelista(indice))
    {
        if (capacita_da_controllare == capacita.get_nome_capacita(indice))
        {
            if (capacita.get_luogo_capacita(indice) == 0)
            {
                ...
                stringa_risposta = "hai letto l'etichetta sotto il pulsante per l' S.O.S. GALATTICO.";
                storia_gioco.insStoria(stringa_comando, stringa_risposta);
            }
            else
                interfaccia.scrivi("Forse ti manca una capacita'...\n");
...
{
    bool premuto = false;
    string capacita_da_controllare = "premere";
    Lista<Capacita_singola> cap = capacita.get_lista_capacita();
    Cella<Capacita_singola> *indice = cap.primolista();
    while (!cap.finelista(indice))
    {
        if (capacita_da_controllare == capacita.get_nome_capacita(indice))
        {
            ...
            {
                premuto = true;
                interfaccia.scrivi("Hai premuto il pulsante.");
            }
            else
                interfaccia.scrivi("Forse ti manca una capacita'...\n");
        }
    ...
    if (premuto)
    {
        capacita_da_controllare = "leggere";
        ...
        while (!cap.finelista(indice))
        {
            if (capacita_da_controllare == capacita.get_nome_capacita(indice))
            {
                if (capacita.get_luogo_capacita(indice) == 0)
                {
                    ...
                    stringa_risposta = "hai appreso un dettaglio dell'ANTENNA ESTERNA.";
                    storia_gioco.insStoria(stringa_comando, stringa_risposta);
                }
            }
            indice = cap.succlista(indice);
    ...
    while (!cap.finelista(indice))
    {
        ...
        if (capacita_da_controllare == capacita.get_nome_capacita(indice))
        {
            if (capacita.get_luogo_capacita(indice) == 0)
            {
                ...
                stringa_risposta = "hai letto il cartello davanti alla cabina di pilotaggio.";
                storia_gioco.insStoria(stringa_comando, stringa_risposta);
            }
            else interfaccia.scrivi("Forse ti manca una capacita'...\n");
        }
        indice = cap.succlista(indice);
    }
}
    ...
    while (!cap.finelista(indice))
    {
        ...
        if (capacita_da_controllare == capacita.get_nome_capacita(indice))
        {
            if (capacita.get_luogo_capacita(indice) == 0)
            {
                ...
                stringa_risposta = "hai letto un avviso davanti al compartimento stagno.";
                storia_gioco.insStoria(stringa_comando, stringa_risposta);
            }
            else
                interfaccia.scrivi("Forse ti manca una capacita'...\n");
...
void Astro::azione_23()
{
    ...
    bool fatto = false;
    bool aprire = false;
    string capacita_da_controllare = "aprire";
    string capacita_da_imparare = "saltare";
    Lista<Capacita_singola> cap = capacita.get_lista_capacita();
    Cella<Capacita_singola> *indice = cap.primolista();
    while (!cap.finelista(indice))
    {
        if (capacita.get_nome_capacita(indice) == capacita_da_imparare && capacita.get_luogo_capacita(indice) == 0)
            trovato = true;
        indice = cap.succlista(indice);
    }
    indice = cap.primolista();
    while (!cap.finelista(indice))
    {
        if (capacita.get_nome_capacita(indice) == capacita_da_controllare && capacita.get_luogo_capacita(indice) == 0)
            aprire = true;
        indice = cap.succlista(indice);
    }
    indice = cap.primolista();
    if (aprire)
    {
        if (oggetti.get_oggetto(23).get_luogo() != -9999)
        {
            interfaccia.scrivi("Gia' fatto.");
            stringa_risposta = "e' stato inutile perche' avevi gia' aperto l'armadietto.";
            storia_gioco.insStoria(stringa_comando, stringa_risposta);
            if (!trovato)
            {
                if (n_capacita_acquisite < 5)
                {
                    while (!cap.finelista(indice) && !fatto)
                    {
                        if (capacita_da_imparare == capacita.get_nome_capacita(indice))
                        {
                            capacita.set_luogo_capacita(capacita.get_nome_capacita(indice), 0);
                            interfaccia.scrivi("Hai appreso la capacita' di saltare!\n");
                            fatto = true;
                    ...
                    interfaccia.scrivi(
                        "Non hai appreso la capacita' di saltare in quanto hai raggiunto il numero massimo di capacita acquisibili! Riprova l'azione dopo che hai eliminato almeno una capacita'!\n");
            }
        }
        else if (oggetti.get_oggetto(24).get_luogo() != 0)
        {
            interfaccia.scrivi("E' chiuso a chiave.");
            bacheca.NuovoMessaggioGioco("*per aprire l'armadietto serve la chiave..");
            stringa_risposta = "hai notato che l'armadietto era chiuso a chiave.";
            storia_gioco.insStoria(stringa_comando, stringa_risposta);
        }
        else
        {
            ...
            stringa_risposta = "l'hai aperto.";
            storia_gioco.insStoria(stringa_comando, stringa_risposta);

            oggetti.set_luogo(25, luogo_attuale);
            oggetti.set_luogo(23, luogo_attuale);
            oggetti.set_luogo(57, luogo_attuale); //aggiunta modifica di Damone Marco (tesserino nell'armadietto del secondo pilota)
            if (!trovato)
            {
                if (n_capacita_acquisite < 5)
                {
        ...
        if (capacita_da_controllare == capacita.get_nome_capacita(indice))
        {
            if (capacita.get_luogo_capacita(indice) == 0)
            {
                if (parete_stagna_aperta == 1)
                {
                    ...
                    stringa_risposta = "hai sentito uno strano rumore...";
                    storia_gioco.insStoria(stringa_comando, stringa_risposta);
                }
                else
                {
                    ...
                    stringa_risposta = "si e' aperta una parete ad ovest, verso la poppa dell'astronave.";
                    storia_gioco.insStoria(stringa_comando, stringa_risposta);
                    ...
                    else
                    {
                        for (i = 1; i <= oggetti.get_n_oggetti(); i++)
                        {
                            ...
                            {
                                interfaccia.scrivi(oggetti.get_oggetto(i).get_nome() + " si perde nello spazio");
                ...
                interfaccia.scrivi("Ti manca la capacita' di premere! Riprova quando avrai ottenuto tale capacita'!");
        }
        indice = cap.succlista(indice);
    }
}
        ...
        if (capacita_da_controllare == capacita.get_nome_capacita(indice))
        {
            if (capacita.get_luogo_capacita(indice) == 0)
            {
                ...
                else
                {
                    ...
                    stringa_risposta = "si e' chiusa la parete ad ovest: la stanza si e' ripressurizzata.";
                    storia_gioco.insStoria(stringa_comando, stringa_risposta);

                    parete_stagna_aperta = 0;
                    if (!(oggetti.get_oggetto(21).get_luogo() != 0 || oggetti.get_oggetto(24).get_luogo() != -9999))
                    {
                        ...
                        oggetti.set_luogo(21, luogo_attuale);
                        oggetti.set_luogo(24, luogo_attuale);
                    }
                }
            }
            else
    ...
    while (!cap.finelista(indice))
    {
        if (capacita_da_controllare == capacita.get_nome_capacita(indice))
        {
            if (capacita.get_luogo_capacita(indice) == 0)
            {
                ...
                stringa_risposta = "ti ha consigliato di portare sempre con te il manuale del reattore!";
                storia_gioco.insStoria(stringa_comando, stringa_risposta);
            }
            else
                interfaccia.scrivi("Forse ti manca una capacita'...\n");
            ...
            if (capacita.get_luogo_capacita(indice) == 0)
            {
                ...
                else
                {
                    ...
                    if (oggetti.get_oggetto(22).get_luogo() != 0)
                    {
                        ...
                        stringa_risposta = "hai cominciato a non sentirti bene...";
                        storia_gioco.insStoria(stringa_comando, stringa_risposta);
                        bacheca.NuovoMessaggioGioco(
...
//inizio modifiche Gravina Antonio
//START DAMONE
void Astro::cerca() // nome sostitutivo azione_123()
{
    Lista<Capacita_singola> cap = capacita.get_lista_capacita();
    Cella<Capacita_singola> *indice = cap.primolista();
    bool trovato = false;
    ...
    {
        if (capacita.get_nome_capacita(indice) == "leggere")
        {
            fine = true;
            if (capacita.get_luogo_capacita(indice) == 0)
            {
    ...
    if (oggetti.get_oggetto(55).get_luogo() == 0)
    {
        ...
        {
            interfaccia.scrivi("Ti manca la capacita' di leggere. E adesso ?");
        }
    }
    else
    {
...
//inizio modifiche Gravina Antonio
//MODIFICA D-R(Colturi):Luoghi a Pagamento
void Astro::azione_63(Mappa &M)
{
    // Capacità usando LISTA: LEGGI INSEGNA LUMINOSA
    string capacita_da_controllare = "leggere";
    Lista<Capacita_singola> cap = capacita.get_lista_capacita();
    Cella<Capacita_singola> *indice = cap.primolista();
    while (!cap.finelista(indice))
...
void Astro::azione_69() //Azione usa terminale nel pronto soccorso
{
    ...
    if (generatore_coda > 0) //Se ci sono persone in coda, chiedi al Giocatore cosa desidera fare
    {
        ...
        {
            cin >> input_utente;
            if (input_utente == "S" || input_utente == "s" || input_utente == "Si" || input_utente == "SI" ||
                    input_utente == "si") //Se l'utente digita si, aggiorna conferma
            {
            ...
            else if (input_utente == "N" || input_utente == "n" || input_utente == "No" || input_utente == "NO" ||
                     input_utente == "no") //Se l'utente digita no, aggiorna conferma
            {
            ...
            else
            {
                ...
                interfaccia.scrivi_parziale(generatore_coda);
                interfaccia.scrivi_parziale(
                    " persone attendono di poter usare il terminale. Desideri aspettare? [si/no] ");
                interfaccia.a_capo();
            }
        }
    }
    ...
    {
        while (!cap.finelista(indice) && esegui < 5)
        {
            ...
            else if ("digitare" == capacita.get_nome_capacita(indice))
            {
                if (capacita.get_luogo_capacita(indice) == 0)
                {
                    esegui += 4;
        ...
        if (esegui == 8)
        {
            ...
            //system("cls"); // for windows os - Modificato da SIMONE VERSIENTI
            ...
            SLEEP(850); //Attendi alcuni istanti - Modificato da SIMONE VERSIENTI
        ...
        else
        {
            ...
            {
                interfaccia.scrivi(
                    "Ti manca la capacita' di digitare, hai sprecato tempo.Riprova quando avrai ottenuto tale capacita'!");
            }
            else if (esegui == 6)
            {
                interfaccia.scrivi(
                    "Ti manca la capacita' di leggere, hai sprecato tempo.Riprova quando avrai ottenuto tale capacita'!");
            }
            else if (esegui == 5)
            {
                interfaccia.scrivi(
                    "Ti manca la capacita' di leggere e digitare, hai sprecato tempo.Riprova quando le avrai ottenute entrambe!");
            }
        }
    }
}
...
void Astro::azione_82()
{
    ...
    if (strcmp(risp.c_str(), "s") == 0)
    {
        ...
        if (esegui == 8)
        {
            ...
            SLEEP(920); // - modificato da SIMONE VERSIENTI
            //system("cls");
            //system("clear"); //linux os  //Modificato da ANTONIO PASTORELLI
            CLEARSCREEN(); // inserito da SIMONE VERSIENTI per compatibilità Linux/Windows
            interfaccia.scrivi("\n+--------------------------------------------------------------+");
            ...
            while (uscita != true)
            {
                    ...
                    while (uscitainvia != true)
                    {
                        ...
                        case 1:
                            if (ufficiopostale.get_Suggerimento() == true)
                            {
                                ...
                                interfaccia.scrivi(
                                    "\n+-------------------------------------------------------------+");
                                interfaccia.scrivi(
                                    "+ Attenzione! Hai gia' richiesto il suggerimento!             +");
                                interfaccia.scrivi(
                                    "+ Non puoi utilizzare nuovamente questa funzione              +");
                                interfaccia.scrivi(
                                    "+-------------------------------------------------------------+");
                            }
                            else
                            {
                                ...
                                interfaccia.scrivi(
                                    "+ Riceverai a breve la lettera.                                +");
                                interfaccia.scrivi(
                                    "+ Controlla nella sezione di ricezione.                        +");
                                interfaccia.scrivi(
                                    "+--------------------------------------------------------------+");
                            }
                            uscitainvia = true;
                        ...
                        case 2:
                            if (ufficiopostale.get_Pacco() == true)
                            {
                                ...
                                interfaccia.scrivi(
                                    "\n+-------------------------------------------------------------+");
                                interfaccia.scrivi(
                                    "+ Attenzione! Hai gia' richiesto il pacco!                    +");
                                interfaccia.scrivi(
                                    "+ Non puoi utilizzare nuovamente questa funzione              +");
                                interfaccia.scrivi(
                                    "+-------------------------------------------------------------+");
                        ...
                        case 3:
                            ...
                            interfaccia.scrivi(
                                "\n+-------------------------------------------------------------+");
                            interfaccia.scrivi(
                                "+ Scegli dall'inventario l'oggetto da inviare.                +");
                            interfaccia.scrivi(
                                "+ Inserendo il relativo codice spedizione.                    +");
                            for (int i = 1; i <= oggetti.get_n_oggetti(); i++)
                            {
                            ...
                            cin >> spedizione;
                            interfaccia.scrivi(
                                "+ Inserisci il codice del luogo di destinazione.              +");
                ...
                case 2:
                    if (oggetti.get_oggetto(74).get_luogo() == 0) //get_oggetto(x,y) x da 80 a 81 Galeandro
                    {
                        //system("cls");
                        //system("clear"); //linux os  //Modificato da ANTONIO PASTORELLI
                        CLEARSCREEN(); // inserito da SIMONE VERSIENTI per compatibilità Linux/Windows
                        ...
                        if (ufficiopostale.get_Suggerimento() == true &&
                                ufficiopostale.get_ritiratoSuggerimento() == false)
                        {
        ...
        while (!cap.finelista(indice) && trovato == false)
        {
            if (capacita_da_controllare == capacita.get_nome_capacita(indice))
            {
                ...
                if (capacita.get_luogo_capacita(indice) == 0)
                {
                    interfaccia.scrivi("Messaggio:");
                    interfaccia.scrivi(
                        "La chiave dell'armadietto del secondo pilota potrebbe essere andata perduta!. . .");
                    interfaccia.scrivi("Per fortuna che alla base hanno sempre i pezzi di ricambio! ");
                }
                else
                {
                    interfaccia.scrivi(
                        "Ma ti manca la capacita' di leggere! Riprova quando avrai ottenuto tale capacita'!");
                }
            }
            indice = cap.succlista(indice);
        }
...
void Astro::azione_91()
{
    ...
    string capacita_da_controllare = "digitare";
    Lista<Capacita_singola> cap = capacita.get_lista_capacita();
    Cella<Capacita_singola> *indice = cap.primolista();
            ...
            if (capacita.get_luogo_capacita(indice) == 0)
            {
                ...
                while (!uscita_computer)
                {
                    ...
                    interfaccia.scrivi(
                        " -------------------------------------------------------------------------------");
                    interfaccia.scrivi(
                        "|Sistema Archivio 2.0                                                           |");
                    interfaccia.scrivi(
                        "|Benvenuto Comandante nel sistema di archivio dell'astronave.                   |");
                    interfaccia.scrivi(
                        "|A causa dei gravi danni subiti, l'integrita' dell'archivio e' compromessa.     |");
                    interfaccia.scrivi(
                        "|Il sistema e' stato in grado di ripristinare solo alcuni file.                 |");
                    interfaccia.scrivi(
                        "|Inoltre vi e' un problema nella visualizzazione a schermo dei file.            |");
                    interfaccia.scrivi(
                        "|I file possono pero' essere stampati senza problemi                            |");
                    interfaccia.scrivi(
                        "|Per ritirare il documento digitare il numero corrispondente.                   |");
                    interfaccia.scrivi(
                        "|Attenzione: problemi tecnici rallenteranno il processo di stampa.              |");
                    interfaccia.scrivi(
                        "|Per uscire digitare il numero corrispondente.                                  |");
                    interfaccia.scrivi(
                        "|1 <- File12                                                                    |");
                    interfaccia.scrivi(
                        "|2 <- File13                                                                    |");
                ...
                interfaccia.scrivi("Ti manca la capacita' di digitare! Riprova quando avrai ottenuto tale capacita'!");
        }
        indice = cap.succlista(indice);
    }
}
        ...
        if (capacita_da_controllare == capacita.get_nome_capacita(indice))
        {
            if (capacita.get_luogo_capacita(indice) == 0)
            {
                ...
                else
                {
                    ...
                    interfaccia.scrivi(
                        "L'equipaggio non e' a conoscenza di particolari rischi derivanti dallo scorretto utilizzo dei sistemi di bordo.");
                ...
                interfaccia.scrivi("Ti manca la capacita' di leggere! Riprova quando avrai ottenuto tale capacita'!");
        }
        ...
        if (capacita_da_controllare == capacita.get_nome_capacita(indice))
        {
            if (capacita.get_luogo_capacita(indice) == 0)
            {
                ...
                else
                {
                    ...
                    interfaccia.scrivi(
                        "Un cadetto ha effettuato l'altro giorno delle riparazioni fuori dall'astronave");
        ...
        if (capacita_da_controllare == capacita.get_nome_capacita(indice))
        {
            if (capacita.get_luogo_capacita(indice) == 0)
            {
                ...
                else
                {
                    ...
                    interfaccia.scrivi(
                        "A quanto pare uno dei cadetti aveva prodotto delle osservazioni poco lusinghiere su un collega.");
        ...
        if (capacita_da_controllare == capacita.get_nome_capacita(indice))
        {
            if (capacita.get_luogo_capacita(indice) == 0)
            {
                ...
                else
                {
                    ...
                    interfaccia.scrivi(
                        "Il reattore dell'astronave ha fatto registrare dei valori anomali negli ultimi giorni.");
                    interfaccia.scrivi(
                        "Ne ho parlato con il comandante e sembra d'accordo nell'avviare un'indagine approfondita.");
...
void Astro::azione_97()
{
    ...
    while(!cap.finelista(indice) && trovato==false)
    {
        if(capacita_da_controllare == capacita.get_nome_capacita(indice))
        {
            ...
            else interfaccia.scrivi ("Ti manca la capacita' di leggere! Riprova quando avrai ottenuto tale capacita'!");
        }
        indice = cap.succlista(indice);
    }
}
...
void Astro::azione_100()
{
    ...
    if (!gioco_preside)
    {
        ...
        {
            while (!cap.finelista(indice) && trovato < 5)
            {
                if ("leggere" == capacita.get_nome_capacita(indice))
                {
                    if (capacita.get_luogo_capacita(indice) == 0)
                ...
                else if ("scrivere" == capacita.get_nome_capacita(indice))
                {
                    if (capacita.get_luogo_capacita(indice) == 0)
                    {
                        trovato += 3;
            ...
            else
            {
                if (trovato == 6)
                {
                    interfaccia.scrivi("Ti manca la capacita' di scrivere. Ritorna quando l'avrai imparata.");
                }
                else if (trovato == 7)
                {
                    interfaccia.scrivi("Ti manca la capacita' di leggere. E adesso come farai ?");
                }
                else if (trovato == 8)
                {
                    interfaccia.scrivi("Ti manca la capacita' di leggere e scrivere. E adesso ?");
                }
            }
    ...
    if (!gioco_prof_clara)
    {
        ...
        if (risp == "Si" || risp == "s" || risp == "si")
        {
            ...
            else
            {
                ...
                {
                    interfaccia.scrivi("Ti manca la capacitì di leggere e scrivere. E adesso ?");
                }
            }
        }
        else
...
void Astro::azione_115()
{
    ...
    */
    ...
    if(oggetti.get_oggetto(26).get_luogo() == 0)//Controllo se il personaggio ha il portafoglio
    {
        while(input!="si" && input!="s" && input!="Si" && input!="SI" && input!="no" && input!="NO" && input !="No" && input!="n" && input!="N")
        {
            ...
            if(input=="si" || input=="s" ||input=="S" || input=="Si"||input=="SI" || input=="sI")
            {
                ...
                if (trovato == 5)
                {
                    cout << "\n\n\n";
                    interfaccia.scrivi(
                        "########################################################");//messaggio di output per la biglietteria
                    interfaccia.scrivi("########################################################");
                ...
                else
                {
                    if (trovato == 6)
                    {
                        interfaccia.scrivi("Ti manca la capacita' di digitare. Ritorna quando l'avrai imparata.");
                    }
                    else if (trovato == 7)
                    {
                        interfaccia.scrivi("Ti manca la capacita' di leggere. E adesso come farai ?");
                    ...
                    {
                        interfaccia.scrivi("Ti manca la capacitì di leggere e digitare. E adesso ?");
                    }
                }
                /*
    ...
    else
    {
        if(Macchinetta.appartiene(codiceinserito))
        {
            ...
            if (Macchinetta.recupera(codiceinserito).getQuantita() > 0)
            {
                ...
                {
                    interfaccia.a_capo();
                    interfaccia.scrivi("\nNon hai il portafoglio! Magari devi prima prenderlo da qualche parte!\n");
                }
                else if (portafoglio.get_contanti() >= Macchinetta.recupera(codiceinserito).getCosto())
                {
...
void Astro::azione_136() //leggi diario
{
    ...
    else
    {
        while(!cap.finelista(indice) && trovato==false)
        {
            ...
            else interfaccia.scrivi ("Non sai come leggere! Riprova quando avrai ottenuto tale capacita'!");
        }
        indice = cap.succlista(indice);
    }
}
...
void Astro::azione_137() //guarda cartello
{
    ...
    while (!cap.finelista(indice))
    {
        if (capacita_da_controllare == capacita.get_nome_capacita(indice))
        {
            ...
            {
                interfaccia.scrivi("Non sai leggere!");
            }
        }
        indice = cap.succlista(indice);
    }
    ...
    if (luogo_attuale==12)
    {
        ...
        while (!cap.finelista(indice) && trovato == false)
        {
            if (capacita_da_controllare == capacita.get_nome_capacita(indice))
            {
                ...
                {
                    interfaccia.scrivi(
                        "Per lasciare un documento devi digitare il codice, ma non hai la capacità!");
                }
            }
            indice = cap.succlista(indice);
        }
...
void Astro::azione_141()
{
    ...
    while (!cap.finelista(indice) && trovato == false)
    {
        ...
        {
            interfaccia.scrivi(
                "Per ritirare un documento devi digitare il codice, ma non hai la capacità!");
        }
    }
    indice = cap.succlista(indice);
}
...
//inizio modifiche Gravina Antonio
//inizio modifiche Lentini Giovanni relative all'implementazione delle azioni per il luogo Sali&Tabacchi
...
//system("cls");
                    ...
                    interfaccia.scrivi("\n\t\t%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
                    interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%%%                                        LOTTERIA NEUTRONIA                                         %%%%%%%%%%%%%%%%%%%%%%%\n");
                    interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%%%                                                                                                   %%%%%%%%%%%%%%%%%%%%%%%\n");
                    interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%%%      Istruzioni per giocare:                                                                      %%%%%%%%%%%%%%%%%%%%%%%\n");
                    interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%%%  - Acquista il biglietto (costo 1 euro)                                                           %%%%%%%%%%%%%%%%%%%%%%%\n");
                    interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%%%  - Scegli un numero da 1 a 99                                                                     %%%%%%%%%%%%%%%%%%%%%%%\n");
                    interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%%%  - Se il numero che hai scelto sara' presente tra i numeri della ruota Neutronia vincerai 30 euro %%%%%%%%%%%%%%%%%%%%%%%\n");
                    interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%%%                                                                                                   %%%%%%%%%%%%%%%%%%%%%%%\n");
                    interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
                    interfaccia.scrivi("\nDesideri acqistare il biglietto e tentare la fortuna? [s/n] \n");
                    ...
                    do
                    {
                        ...
                        if((risposta == "s") || (risposta == "si"))
                        {
                            ...
                            float saldo = portafoglio.get_contanti();
                            ...
                            if(saldo >= 1)
                            {
                                ...
                                cout<<"\nInserisci il numero scelto da giocare:"<<endl;
                                do
                                {
                                    cin>>numero_scelto;
                                    if((numero_scelto > 99) || (numero_scelto < 1))
                                    {
                                        cout<<"\nIl numero scelto non e' compreso, inseriscilo nuovamente:"<<endl;
                                    }
                                }
                                while((numero_scelto > 99) || (numero_scelto < 1));

                                lotteria.inpila(numero_scelto);
...
//scontrino lotteria
                                ...
                                interfaccia.scrivi("***     ****   *****        ***\n");
                                interfaccia.scrivi("***                         ***\n");
                                cout<<"***    Numero: "<<numero_scelto<<"           ***"<<endl;
                                interfaccia.scrivi("***                         ***\n");

                                interfaccia.scrivi("*******************************\n");

                                SLEEP(1500); // - modificato da SIMONE VERSIENTI
                                cout<<"\t\t\t\t\t\t\t\t\t\t\t\tCaricamento in corso ";
                                SLEEP(500); // - modificato da SIMONE VERSIENTI
                                cout<<" .";
                                SLEEP(300); // - modificato da SIMONE VERSIENTI
                                cout<<" .";
                                SLEEP(300); // - modificato da SIMONE VERSIENTI
                                cout<<" .\n";
...
//estrazione
                                interfaccia.scrivi("\n\n\t\t%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
                                interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%%%                                   LOTTERIA NEUTRONIA                                          %%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
                                interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%%%                                   Estrazione in corso                                         %%%%%%%%%%%%%%%%%%%%%%%%%%%\n");


                                SLEEP(500); // - modificato da SIMONE VERSIENTI
                                numero_estrazione = rand()% 10 + 61; // da 61 a 70
                                cout<<"\t\t\t\t\t\t\t\t\t\t"<<numero_estrazione<<endl;
                                if(numero_estrazione == numero_scelto)
                                {
                                    controllo_numeri = true;
                                }

                                SLEEP(500); // - modificato da SIMONE VERSIENTI
                                numero_estrazione = rand()% 10 + 1; // da 1 a 10
                                cout<<"\t\t\t\t\t\t\t\t\t\t"<<numero_estrazione<<endl;
                                if(numero_estrazione == numero_scelto)
                                {
                                    controllo_numeri = true;
                                ...
                                SLEEP(500); // - modificato da SIMONE VERSIENTI
                                numero_estrazione = rand()% 10 + 21; // da 21 a 30
                                cout<<"\t\t\t\t\t\t\t\t\t\t"<<numero_estrazione<<endl;
                                if(numero_estrazione == numero_scelto)
                                {
                                    controllo_numeri = true;
                                ...
                                SLEEP(500); // - modificato da SIMONE VERSIENTI
                                numero_estrazione = rand()%10 + 51; // da 51 a 60
                                cout<<"\t\t\t\t\t\t\t\t\t\t"<<numero_estrazione<<endl;
                                if(numero_estrazione == numero_scelto)
                                {
                                    controllo_numeri = true;
                                ...
                                SLEEP(500); // - modificato da SIMONE VERSIENTI
                                numero_estrazione = rand()% 10 + 81; // da 81 a 90
                                cout<<"\t\t\t\t\t\t\t\t\t\t"<<numero_estrazione<<endl;
                                if(numero_estrazione == numero_scelto)
                                {
                                    controllo_numeri = true;
                                ...
                                SLEEP(500); // - modificato da SIMONE VERSIENTI
                                numero_estrazione = rand()% 10 + 31; // da 31 a 40
                                cout<<"\t\t\t\t\t\t\t\t\t\t"<<numero_estrazione<<endl;
                                if(numero_estrazione == numero_scelto)
                                {
                                    controllo_numeri = true;
                                ...
                                SLEEP(500); // - modificato da SIMONE VERSIENTI
                                numero_estrazione = rand()% 9 + 91; // da 91 a 99
                                cout<<"\t\t\t\t\t\t\t\t\t\t"<<numero_estrazione<<endl;
                                if(numero_estrazione == numero_scelto)
                                {
                                    controllo_numeri = true;
                                ...
                                SLEEP(500); // - modificato da SIMONE VERSIENTI
                                numero_estrazione = rand()% 10 + 11; // da 11 a 20
                                cout<<"\t\t\t\t\t\t\t\t\t\t"<<numero_estrazione<<endl;
                                if(numero_estrazione == numero_scelto)
                                {
                                    controllo_numeri = true;
                                ...
                                SLEEP(500); // - modificato da SIMONE VERSIENTI
                                numero_estrazione = rand()% 10 + 41; // da 41 a 50
                                cout<<"\t\t\t\t\t\t\t\t\t\t"<<numero_estrazione<<endl;
                                if(numero_estrazione == numero_scelto)
                                {
                                    controllo_numeri = true;
                                ...
                                SLEEP(500); // - modificato da SIMONE VERSIENTI
                                numero_estrazione = rand()% 10 + 71; // da 71 a 80
                                cout<<"\t\t\t\t\t\t\t\t\t\t"<<numero_estrazione<<endl;
                                if(numero_estrazione == numero_scelto)
                                {
                                    controllo_numeri = true;
                                ...
                                {
                                    interfaccia.scrivi("\n\t\t%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% HAI PERSO! LA PROSSIMA VOLTA SARAI PIU' FORTUNATO %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
                                }

                                interfaccia.scrivi("\n\t\t%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Grazie per aver giocato! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
                                interfaccia.scrivi("\n\t\t%%%%%%%%%%%%%%%%%%%%% Conserva il biglietto! Puoi ancora tentare la fortuna all'interno dell'elaboratore bonus %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");

                            }

                            else
                            {
                                interfaccia.scrivi("\nNon hai abbastanza denaro per acquistare il biglietto");
                            }
                            flag = true;
                        }
                ...
                {
                    interfaccia.scrivi("\nNon puoi giocare alla lotteria se non hai il portafoglio con all'interno almeno 1 euro");
            ...
            {
                interfaccia.scrivi(
                    "Se non sai leggere i numeri non ti conviene giocare al lotto!");
            }
        }
        indice = cap.succlista(indice);
    }
    ...
    do
    {
        ...
        if ((risposta == "s") || (risposta == "si"))
        {
            ...
            if(verifica_denaro)
            {
                ...
                if(conta >1)
                {
                    cout<<"\n Sono stati stampati "<<conta<<" scontrini del prodotto 'sale'"<<endl;
    ...
    do
    {
        ...
        if ((risposta == "s") || (risposta == "si"))
        {
            ...
            if(verifica_denaro)
            {
                ...
                if(conta >1)
                {
                    cout<<"\n\n\t Sono stati stampati "<<conta<<" scontrini del prodotto 'sigarette'"<<endl;
...
//inizio modifiche Gravina Antonio
void Astro::azione_145()  //  bonus-usa elaboratore(bonus)
{
    Lista<Capacita_singola> cap = capacita.get_lista_capacita();
    Cella<Capacita_singola> *indice = cap.primolista();
    int esegui = 0;
    ...
    if (esegui == 8)
    {
        ...
        do
        {
            ...
            if((risposta=="s") ||(risposta == "si"))
            {
                ...
                /*
                interfaccia.scrivi(".....   Il numero di scontrini che puoi convertire e' .....\n");

                cout<<".....                        "<<acquisti.lunghLista()<< "                        .....\n"<<endl;
                */

                interfaccia.scrivi(".....                                                      .....\n");
                do
                {
                    interfaccia.scrivi("..... - Clicca 1 per convertire un solo scontrino          .....\n");
                    interfaccia.scrivi("..... - Clicca 2 per converitre piu' scontrini(max 3)      .....\n");
                    interfaccia.scrivi("..... - Clicca 3 per giocare il biglietto della lotteria   .....\n");
                    interfaccia.scrivi("..... - Clicca 4 per uscire dall'elaboratore  bonus        .....\n");
                    ...
                    case 1:
                        //converti solo uno scontrino

                        if(!acquisti.listavuota())
                        {

                            acquisti.leggilista(punt);
                            totale = ricevuta.get_prezzo() + rand()%11+1;


                            acquisti.canclista(punt);

                            interfaccia.scrivi("...........................................................\n");
                            interfaccia.scrivi(".....                                                 .....\n");
                            interfaccia.scrivi(".....                Tempo residuo:                   .....\n");
                            cout<<".....                      "<<tempo<<"                        .....\n";
                            interfaccia.scrivi("\n.....                                                 .....\n");
                            interfaccia.scrivi(".....                                                 .....\n");
                            interfaccia.scrivi("\n.....         Punti da convertire in bonus            .....\n");
                            cout<<"                          "<<totale<< "                          \n";
                            tempo = tempo + totale;


                            SLEEP(700); // - modificato da SIMONE VERSIENTI
                        ...
                        {
                            interfaccia.scrivi(".....    Non hai scontrini da convertire in punti bonus!    .....\n");
                            interfaccia.scrivi(".................................................................\n");
                            interfaccia.scrivi(".................................................................\n");
                    ...
                    case 2:
                        //converte piï¿½ scontrini

                        interfaccia.scrivi(".....                                                      .....\n");
                        interfaccia.scrivi(".....    Inserisci il numero di scontrini da convertire    .....\n");
                        ...
                        {
                            interfaccia.scrivi(".....                                                      .....\n");
                            interfaccia.scrivi(".....    Il numero inserito non e' valido, perfavore       .....\n");
                            interfaccia.scrivi(".....               inseriscilo nuovamente                 .....\n");
                            cin>>num;
                        }
                        ...
                        for(int i = 0; i< num; i++)
                        {
                            if(!acquisti.listavuota())
                            {
                                ...
                                interfaccia.scrivi("...........................................................\n");
                                cout<<".....  Punti da convertire in bonus del "<<i+1<< " scontrino   .....\n"<<endl;
                                cout<<".....                        "<<totale<< "                       .....\n"<<endl;
                                tempo = tempo + totale ;
                                SLEEP(700); // - modificato da SIMONE VERSIENTI
                                interfaccia.scrivi(".....                                                 .....\n");
                                ...
                                SLEEP(1000); // - modificato da SIMONE VERSIENTI
                    ...
                    case 3:
                        ...
                        else
                        {
                            ...
                            cout<<".....    L'ultimo biglietto comprato era il Numero : "<<lotteria.leggipila()<< ".....\n"<<endl;

                            cout << "\n\n- Premi <invio> per scoprire se hai vinto -\n";
                            interfaccia.attendi();


                            if((lotteria.leggipila() % 2 == 0) || (lotteria.leggipila() % 20 == 0) || (lotteria.leggipila() % 6 == 0) || (lotteria.leggipila() % 15 == 0))
                            {
...
//inizio modifiche Gravina Antonio
void Astro::azione_146()    //vedi-leggi-guarda orari
{
    string capacita_da_controllare = "leggere";
    Lista<Capacita_singola> cap = capacita.get_lista_capacita();
    Cella<Capacita_singola> *indice = cap.primolista();
...
//system("cls");
                CLEARSCREEN(); // inserito da SIMONE VERSIENTI per compatibilità Linux/Windows
                interfaccia.scrivi("\n\n\n\n\t\t%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
                interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%    Ore 8.30 Aggiornamento programma         %%%%%%%%%%%%%%%%%%%\n");
                interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%    Ore 9.30  Saluto dal Capitano            %%%%%%%%%%%%%%%%%%%\n");
                interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%    Ore 10.00 Servizio Colazione             %%%%%%%%%%%%%%%%%%%\n");
                interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%    Ore 11.00 Intrattenimento bambini =)     %%%%%%%%%%%%%%%%%%%\n");
                interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%    Ore 12.45 Servizio pranzo                %%%%%%%%%%%%%%%%%%%\n");
                interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%    Ore 15.00 Evento 'SU NELLO SPAZIO =)'    %%%%%%%%%%%%%%%%%%%\n");
                interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%    Ore 17.00 Apertura attivita' sportive    %%%%%%%%%%%%%%%%%%%\n");
                interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%    Ore 18.20 Evento  'NeutroniaGalaxy'      %%%%%%%%%%%%%%%%%%%\n");
                interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%    Ore 19.30 Servizio Cena                  %%%%%%%%%%%%%%%%%%%\n");
                interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%    Ore 21.30 Spettacolo serale              %%%%%%%%%%%%%%%%%%%\n");
                interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%    Ore 23.15 Saluto finale e  gadget        %%%%%%%%%%%%%%%%%%%\n");
                interfaccia.scrivi("\t\t%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
...
void Astro :: azione_157 ( )
{
    ...
    default_random_engine rngnumerico(seed);
    uniform_int_distribution <int>rangediabete(20, 400);
    uniform_int_distribution <int>rangecolesterolo(90, 200);
    int risposta_documento;
    bool uscita_computer= false;
    // Menu di scelta 0,1,...

    interfaccia.scrivi(" -------------------------------------------------------------------------------");
    interfaccia.scrivi("Sono il droide medico della nave Neutronia");
    interfaccia.scrivi("Quali analisi del sangue vorresti fare?");
    interfaccia.scrivi("1 <- Diabete");
    interfaccia.scrivi("2 <- Colesterolo");
    interfaccia.scrivi("3 <- Saluta il droide");
    interfaccia.scrivi(" -------------------------------------------------------------------------------\n");
    ...
    {
    case 1:
        diabete = rangediabete(rngnumerico);
        interfaccia.scrivi("Diabete: ");
        cout<<diabete<<endl;
        if(diabete<=60)
        {
            interfaccia.scrivi("Sei in ipoglicemia");
        }
        else if(diabete>60&& diabete<100)
        {
            interfaccia.scrivi("Il tuo livello di glicemia è nella norma");
        }
        else if(diabete>=100&&diabete<200)
        {
            interfaccia.scrivi("La tua glicemia è alterata o leggermente alta");
        }
        else if(diabete>200)
        {
            interfaccia.scrivi("Sei in iperglicemia");
        }
        break;


    case 2:
        colesterolo = rangecolesterolo(rngnumerico);
        interfaccia.scrivi("Colesterolo: ");
        cout<<colesterolo<<endl;


        if(colesterolo<130)
        {
            interfaccia.scrivi("Il livello del colesterolo è normale");
        }
        else if(colesterolo>130&& diabete<160)
        {
            interfaccia.scrivi("Il livello del colesterolo è alterato o leggermente alto");
        }

        else if(colesterolo>160)
        {
            interfaccia.scrivi("Il livello del colesterolo è alto");
        }
        break;
    case 3:
        uscita_computer=true;
...
//INIZIO modifiche De Florio Cristina
void Astro::azione_158(Mappa &M)
{
    int a;
    int j;

    cout<<"Quale mappa vuoi visualizzare?" <<endl;
    cout<<"1)Luoghi adiacenti a quello attuale" << "           "<<"2) Mappa intera" <<endl;
    cin>>a;
    cout<< endl;

    string dir[] = {"N","S","E","W","U","D"};

    switch (a)
    {
    case 1:
        cout<< "LUOGO ATTUALE -> " << mappa.get_nome_luogo(luogo_attuale) <<endl;
        for(j=1; j<7; j++)
        {
            if (mappa.luogo_adiacente(luogo_attuale,j)!=0)
                cout<< dir[j-1] << ": "<< mappa.get_nome_luogo(mappa.luogo_adiacente(luogo_attuale,j))<< "       ";
        }
        cout<< endl;
        break;
    ...
    default:
        ...
        for (int i; i<M.get_numnodi()-1; i++)
        {
            cout<< "LUOGO -> " << mappa.get_nome_luogo(i) <<endl;
            for(j=1; j<7; j++)
            {
                if(mappa.luogo_adiacente(i,j)!=0)
                    cout<< dir[j-1] <<": "<<mappa.get_nome_luogo(mappa.luogo_adiacente(i,j))<< "     ";
            }
            cout<< endl << "-------------------------------------------------------------------------------------------------" << endl;
        }
        break;
...
//inzio modifiche Costantini Andrea
Cucina cucina = Cucina();
void Astro::azione_159()
{
    int k=oggetti.get_oggetto_indice_by_codice(678);
    if (oggetti.get_oggetto(k).get_luogo() == 0)
    {
        interfaccia.scrivi("Hai mangiato la barretta energetica");
        tempo=tempo-20;
        Salute.SetStatoSalute(Salute.GetStatoSalute()+10);
        interfaccia.scrivi("Hai perso 20 secondi per mangiare la barretta energetica");
        interfaccia.scrivi("Hai guadagnato 5 punti punti salute");
    ...
    else
    {
        if (luogo_attuale==49)
        {
            interfaccia.scrivi("Per mangiare la barretta energetica, devi prima prenderla");
        }
        else
        {
            if (oggetti.get_oggetto(k).get_luogo()==-99)
            {
                interfaccia.scrivi("Hai gia' mangiato la barretta, prova a mangiare qualcos'altro");
            }
            else
            {
                interfaccia.scrivi("Se vuoi la barretta energetica devi andare in cucina");
            }
        }
    }
    fflush(stdout);
...
{
    int k=oggetti.get_oggetto_indice_by_codice(679);
    if (oggetti.get_oggetto(k).get_luogo() == 0)
    {
        interfaccia.scrivi("Hai mangiato la pizza");
        tempo=tempo-20;
        Salute.SetStatoSalute(Salute.GetStatoSalute()+5);
        interfaccia.scrivi("Hai perso 20 secondi per mangiare la pizza");
        interfaccia.scrivi("Hai guadagnato 5 punti punti salute");
        oggetti.set_luogo(k,-99);
    }
    else
    {
        if (luogo_attuale==49)
        {
            interfaccia.scrivi("Per mangiare la pizza, devi prima prenderla");
        }
        else
        {
            if (oggetti.get_oggetto(k).get_luogo()==-99)
            {
                interfaccia.scrivi("Hai gia' divorato la pizza, prova a mangiare qualcos'altro");
            }
            else
            {
                interfaccia.scrivi("Se hai voglia di pizza devi andare in cucina");
            }
        }
    }
    fflush(stdout);
...
void Astro::azione_161()
{
    ...
    if (k==1)
    {
        tempo=tempo-10;
        Salute.SetStatoSalute(Salute.GetStatoSalute()+5);
        interfaccia.scrivi("Hai perso 10 secondi");
        interfaccia.scrivi("Hai guadagnato 5 punti salute");
    }
...
{
    cucina.guardaTelevisione();
    Salute.SetStatoSalute(Salute.GetStatoSalute()-3);
    interfaccia.scrivi("Hai perso 3 punti salute");
}

void Astro::azione_163()
{
    cucina.preparaCibo();
}

void Astro::azione_164()
{
    int k=oggetti.get_oggetto_indice_by_codice(677);
    if (oggetti.get_oggetto(k).get_luogo() == 0)
    {
        interfaccia.scrivi("Hai mangiato la mela");
        tempo=tempo-20;
        Salute.SetStatoSalute(Salute.GetStatoSalute()+5);
        interfaccia.scrivi("Hai perso 10 secondi per mangiare la mela");
        interfaccia.scrivi("Hai guadagnato 5 punti punti salute");
        oggetti.set_luogo(k,-99);
    }
    else
    {
        if (luogo_attuale==49)
        {
            interfaccia.scrivi("Per mangiare la mela, devi prima prenderla");
        }
        else
        {
            if (oggetti.get_oggetto(k).get_luogo()==-99)
            {
                interfaccia.scrivi("Hai gia' mangiato la mela, o forse l'hai ceduta a qualcuno");
            }
            else
            {
                interfaccia.scrivi("Se hai voglia di mela devi andare in cucina");
            }
        }
    }
    fflush(stdout);
...
//inizio modifiche Baso Matteo
void Astro::azione_165()
{
    int numOggetti = oggetti.get_n_oggetti();
    Oggetto og;
    for(int i = 0; i < numOggetti; i++)
    {
        og = oggetti.get_oggetto(i);
        cout << i << ") Codice: "<< og.get_codice() <<"; Nome: " << og.get_nome() << "; Luogo: " << og.get_luogo() << "\n";
    }
}
//fine modifiche Baso Matteo
...
void Astro::azione_167() //elimina capacità
{
    ...
    while (!cap.finelista(indice) && !trovato)
    {
        ...
        {
            if (salito == true) interfaccia.scrivi("Non puoi dimenticare come si guida mentre stai guidando! Scendi dal motorino prima\n");
            else
            {
...
{
    if (oggetti.get_oggetto(og).get_luogo() == 0 || oggetti.get_oggetto(og).get_luogo() == luogo_attuale)
    {
        bool trovato = false;
...
void Astro::azione_169()   //leggi foglio per l'impiccato
{
    ...
    while (!cap.finelista(indice) && trovato == false)
    {
        if (capacita_da_controllare == capacita.get_nome_capacita(indice))
        {
            ...
            else interfaccia.scrivi("C'e' qualcosa scritto... potrebbe essere interessante ma ti manca la capacita' di leggere! Riprova quando avrai ottenuto tale capacita'!");
        }
...
void Astro::azione_170()     //guarda poster
{
    ...
    Cella<Capacita_singola> *indice = cap.primolista();
    int trovato = 0;
    while (!cap.finelista(indice) && trovato == 0)                  //Se leggere non è nelle mie capacità, non posso leggere ciò che è scritto sul poster
    {
        if ("leggere" == capacita.get_nome_capacita(indice))
        {
            if (capacita.get_luogo_capacita(indice) == 0)
    ...
    if (trovato == 1)
    {
        ...
        interfaccia.scrivi("- esistono alcune azioni chiamate capacita' che possono essere apprese solo in alcuni luoghi -");
        ...
        int esiste = 0;
        while (!cap.finelista(indice) && esiste == 0)
        {
            if (capacita_da_controllare == capacita.get_nome_capacita(indice))
            {
        ...
        else
        {
            ...
            else interfaccia.scrivi("Non hai appreso la capacita' di " + capacita_da_controllare + " perche' hai raggiunto il numero massimo di capacita' apprese.\n");
        }
    }
    else interfaccia.scrivi("Leggere e' una delle capacita' piu' importanti del gioco, e hai deciso di eliminarla. E adesso come te la caverai?");
...
void Astro::azione_171()  //parla al vagabondo
{
    ...
    if ((risp == "si" || risp == "s" || risp == "Si") && (n_capacita_acquisite < 5))
    {
        ...
        {
            oggetti.set_luogo(k, -99);
            interfaccia.scrivi("Visto che mi hai aiutato ti ricompensero'. Scegli fra una di queste capacita' base:");
            ...
            while (!cap.finelista(indice) && !trovato)
            {
                ...
                {
                    trovato = true;
                    if (capacita.get_luogo_capacita(indice) == 0) interfaccia.scrivi("Hai gia' questa capacita' base. Se vuoi parlarmi di nuovo porta altre mele!");
                    else
                    {
                        n_capacita_acquisite++;
