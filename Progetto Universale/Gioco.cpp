...
//#include <windows.h> // su Linux OS si rompe il build - Modificato da SIMONE VERSIENTI
//#include <unistd.h> //libreria standard per sostituire Sleep(essendo prettamente di windows) commentata da SIMONE VERSIENTI
...
void Gioco::aggiorna_tempo()
{
    ...
    else
    {
        ...
        {
            cout << "Tempo residuo: " << tempo << endl;
            cout << "Tempo aggiuntivo dovuto allo Stato di Salute: " << Salute.GetPenalita() << endl;


        }
        else if(Salute.Ferito()==false)  //MODIFICA LEANDRA PALEMBURGI
        {
...
bool Gioco::direzioni(Mappa &M)
{
    ...
    if (a == 0)
    {
        ...
        percorso.fuoripila();//MODIFICA SPECCHIA ROBERTO
    }
    else
    {
...
void Gioco::prendi()
{
    ...
    {
        if (oggetti.get_oggetto(og).get_codice() == oggetti.get_oggetto(tasca.leggilista(riferimento_tasca)).get_codice())
            trovato = true;
    ...
    else if ( og == 27 || (og >=36 && og <= 44))
    {
        ...
        if (port &&(og>=36 && og <=44) )
        {
            ...
            interfaccia.scrivi(" Non e' Possibile prendere denaro se non hai gia' preso il portafoglio !" );
        else if (!port &&(og==27))
            interfaccia.scrivi("Non e' Possibile prendere la carta di credito  se non hai gia' preso il portafoglio !") ;
        else if (port &&(og==27))
        {
            oggetti.set_luogo(og,0);
    ...
    else if (og == 49 || og == 118 || og == 73) 								                //non puo prendere: motorino
    {
        interfaccia.scrivi ("Quest'oggetto non e' tuo. Per averlo dovresti rubarlo!"); //  chiave_auto
        interfaccia.a_capo();              											 //  buono pasto giallo
        oggetti.set_rubato(og,false);
    }
    //fne modifiche ROSA CHIARAPPA
    else if (!prendi_specifiche())
    {
        if(og == 74)     // Modifica documento d'identità //
        {
            oggetti.set_luogo(og, 0);
            tasca.inslista(og, riferimento_tasca);
        }
        //INZIO MODIFICHE FLAVIO COLLOCOLA
        else if (og == 72 || og == 143)
        {
            oggetti.set_luogo(og, 0);
            tasca.inslista(og,riferimento_tasca);
...
//Fine Modifica Gallone
    ...
    } // modifica Gallone - spostamento parentesi per correzione di un errore che si generava nel momento in cui
    // si prendeva un oggetto che non poteva esser preso
...
void Gioco::perquisizione_giocatore(int &luogo_attuale)
{
    ...
    if (oggetti.get_oggetto(118).get_rubato() == true || oggetti.get_oggetto(73).get_rubato() == true || oggetti.get_oggetto(49).get_rubato() == true) //controllo se l'oggetto
    {
        ...
        SLEEP(30); // - modificato da SIMONE VERSIENTI
...
void Gioco::lascia()
{
    ...
    if (og != 44 && (luogo_attuale != 21 && luogo_attuale != 22))
    {
        ...
        else if (!lascia_specifiche())
        {
            ...
            if (oggetti.get_oggetto(og).get_rubato()==true)  //modifica ROSA CHIARAPPA (setto a folse gli oggetti rubati lasciati)
            {
                oggetti.set_rubato(og,false);
            }
...
void Gioco::guarda()
{
    //if(luogo_attuale==6) luogo_attuale++; // Vaira - Rimosso: l'azione "guarda" eseguita su qualunque oggetto della stanza iniziale
...
void Gioco::resoconto()
{
    if(!Luogo_p.pilavuota())
    {
        Pila <string> app;
        cout << "\nRESOCONTO LUOGHI VISITATI" << endl;
        while ( !Luogo_p.pilavuota() )
        {
            cout << "- " << Luogo_p.leggipila() <<endl;
            app.inpila(Luogo_p.leggipila());
            Luogo_p.fuoripila();
        }
        while ( !app.pilavuota() )
        {
            Luogo_p.inpila(app.leggipila());
            app.fuoripila();
        }
...
void Gioco::ritorna(Pila <stato_comando>* sc, Mappa &M)
{

    if (!percorso.pilavuota())
    {

        string com= percorso.leggipila();
...
//FINE MODIFICHE LEANDRA PALEMBURGI
        ...
        else if( com.compare("sali") == 0 ||
                 com.compare("SALI") == 0 ||
                 com.compare("Sali") == 0 )
        {
            com="scendi";
        }
        else if( com.compare("scendi") == 0 ||
                 com.compare("SCENDI") == 0 ||
                 com.compare("Scendi") == 0 )
        {
            com="sali";
        }
        /* TEST STAMPA con pila di appoggio (app)
                    cout << com << "<- Ultimo invertito \n\n" << endl;
                    Pila <string> app;
                    while (!percorso.pilavuota())
                    {
                    cout << percorso.leggipila() << "<-Pila\n" <<endl;
                    app.inpila(percorso.leggipila());
                    percorso.fuoripila();
                    }
                    while (!app.pilavuota())
                    {
                    percorso.inpila(app.leggipila());
                    app.fuoripila();
                    }
        */
        percorso.fuoripila();
        interfaccia.scrivi("Hai scelto di tornare tornare di un luogo indietro");
        interfaccia.scrivi("\n\n");
        // string stato_comando1 = p->leggipila().get_comando();
        Stato stato= sc->leggipila().get_stato();


        interpreta(com,M,sc,stato,false);
        storia_gioco.insStoria("ritorna", stringa_risposta);
    }
    else
    {
        interfaccia.scrivi("Non puoi tornare indietro, non hai ancora iniziato nessun percorso");
        interfaccia.scrivi("\n\n");
    }
}
...
/* 2130 - 2150 ; 2160 - 2180
	8: ; 9: vuota
*/
/* sostituito da "ritorna" - MODIFICA SPECCHIA
...
void Gioco::switch_enigmi(int a, Mappa &M, Pila<stato_comando>* p, Personaggi* pg)   // modifica zagaria -- aggiunto il paramentro per la gestione dei personaggi
{
    ...
    if(leggienigma())
    {
        ...
        case 8:
            ...
            break;
        case 9:
            ritorna(p,M); //MODIFICA SPECCHIA ROBERTO
            break;
        case 12:
            resoconto(); //MODIFICA PALEMBURGI LEANDRA
            break;
        case 39:
            macchinadeltempo();
            break;
...
void Gioco::esegui(int a, Mappa &M, Pila<stato_comando>* p, bool si, Personaggi* pg) // MODIFICA zagaria -- modifica della funzione esegui con l' aggiunta del parametro personaggi
{
    ...
    else
    {
        ...
        case 9:
            ritorna(p,M); //MODIFICA SPECCHIA ROBERTO
            break;
        case 10:     //modifiche CHIARAPPA ROSA
            ruba();
            break;
    ...
    do
    {
        ...
        do   // ciclo di gioco
        {
            ...
            if((!Luogo_p.pilavuota()) && (Codice_luogo.leggipila()!=luogo_attuale))                                                                                     //MODIFICA POLIMENA
            {
                ...
                cout<<"Provieni dal luogo: " << Codice_luogo.leggipila() << ": "<< Luogo_p.leggipila()<<endl;            // MODIFICA POLIMENA
            }

            Luogo_p.inpila(mappa.get_nome_luogo(luogo_attuale));                                                            //MODIFICA POLIMENA
            Codice_luogo.inpila(luogo_attuale);
            //MODIFICA POLIMENA
...
//INIZIO MODIFICHE - SPECCHIA ROBERTO
            if (
                comando.compare("n") == 0 ||
                comando.compare("nord") == 0 ||
                comando.compare("N") == 0 ||
                comando.compare("NORD") == 0 ||
                comando.compare("Nord")== 0 ||
                comando.compare("o") == 0 ||
                comando.compare("ovest") == 0 ||
                comando.compare("O") == 0 ||
                comando.compare("OVEST") ==0 ||
                comando.compare("Ovest") == 0  ||
//INIZIO MODIFICHE LEANDRA PALEMBURGI
                comando.compare("w") == 0 ||
                comando.compare("west") == 0 ||
                comando.compare("W") == 0 ||
                comando.compare("WEST") ==0 ||
                comando.compare("West") == 0  ||
//FINE MODIFICHE LEANDRA PALEMBURGI
                comando.compare("e") == 0 ||
                comando.compare("est") == 0 ||
                comando.compare("E") ==0 ||
                comando.compare("EST") ==0 ||
                comando.compare("Est") ==0  ||
                comando.compare("s") == 0 ||
                comando.compare("sud") == 0 ||
                comando.compare("S") == 0 ||
                comando.compare("SUD") == 0 ||
                comando.compare("Sud") == 0 ||
                comando.compare("sali") == 0 ||
                comando.compare("SALI") == 0 ||
                comando.compare("Sali") == 0 ||
                comando.compare("scendi") == 0 ||
                comando.compare("SCENDI") == 0 ||
                comando.compare("Scendi") == 0 )
            {
                percorso.inpila(comando);
            }
            //FINE MODIFICHE SPECCHIA ROBERTO
...
void Gioco::navigatore(Mappa& M)
{
    ...
    SLEEP(850); // - modificato da SIMONE VERSIENTI
    interfaccia.scrivi("Connessione avvenuta");
    SLEEP(450); // - modificato da SIMONE VERSIENTI
    //system("cls");
    //system("clear"); //linux os  //Modificato da ANTONIO PASTORELLI
    CLEARSCREEN(); // inserito da SIMONE VERSIENTI per compatibilità Linux/Windows
        ...
        while(!cap.finelista(indice))
        {
            if (capacita_da_controllare == capacita.get_nome_capacita(indice))
            {
                if (capacita.get_luogo_capacita(indice) == 0)
                {
                    ...
                    interfaccia.scrivi("Non puoi leggere! Perche' hai voluto scordare questa capacita'? E adesso?\n");
            }
            indice = cap.succlista(indice);
        }
    }
...
bool Gioco::verifica_den(int elemento)
{
    ...
    if(verifica)
    {
        ...
        {
            interfaccia.scrivi("\nNon hai contanti a sufficienza per acquistare il prodotto");
            verifica = false;
        }
    }
    else
    {
        ...
        {
        case 2:
            interfaccia.scrivi("\nPer poter effettuare acquisti devi avere il portafoglio con almeno 2 euro all'interno ");
            verifica = false;
            break;
        ...
        case 5:
            interfaccia.scrivi("\nPer poter effettuare acquisti devi avere il portafoglio con almeno 5 euro all'interno ");
            verifica = false;
            break;

        default:
            interfaccia.scrivi("\nPer poter effettuare acquisti devi avere il portafoglio con del denaro all'interno ");
            verifica = false;
        }
    }
    return verifica;
