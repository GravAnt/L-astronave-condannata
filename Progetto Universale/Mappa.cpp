...
Mappa::Mappa(ifstream &f,fstream &crono)
{
    ...
    //non controllo l'esito dell'apertura del file, perch� istanzio l'oggetto soltanto se dal main il file � stato aperto correttamente
    int n=0;
    string buffer;
    ...
    while (i<n) //inserimento degli n nodi nella mappa
    {
        ...
        //MODIFICA D-R(Colturi):Luoghi a Pagamento //  controllo quando l' ID � maggiore di 15, devo caricare le descrizioni
        if(e.leggiID()>15)
        {
            e.getDescrizione();
        }
        mappa->scrivinodo(e,*node);         //associo l'etichetta al nodo nel grafo
        //e.inizioDescrizioni();            // Vaira - Considerata la nuova classe per la gestione delle descrizioni non c'� pi� la necessit� di utilizzare questo operatore
...
void Mappa::scrivipartenza() //setta il luogo di partenza
{
    ...
    if(input != "&") //se input == "&" allora la partenza � stata settata dalla cronologia e quindi � inutile rifare tutto il procedimento
    {
        //Fine modifica
        int civ;
        ...
        if (temp.getNodo()!=fittizio->getNodo())
...
void Mappa::scriviarrivo() //analogo a scrivipartenza()
{
    ...
    if(input != "&") //se input == "&" allora la destinazioe � stata settata dalla cronologia e quindi � inutile rifare tutto il procedimento
    {
        int civ;
        Grafo<etichetta,peso>::nodo temp;
        temp=nodo_da_input(input);
...
Grafo<Mappa::etichetta,Mappa::peso>::nodo Mappa::nodo_da_input(string pattern) //restituisce un nodo ricevendo in input (parte del) l'etichetta
{
    ...
    string testo; //     +-- comodo
    int num=0; //--------+
    Lista<riferimento> risultati; //lista che conterr� i risultati
    ...
    if (num>0) //se ho almeno un risultato
    {
        while (scelta!=0)
        {
            CLEARSCREEN(); //Modifica Samuele Pesce per porting universale
            fflush(stdin);
...
{
   // (inizio integrazione Spina)
    if (!(arrivo.getNodo()==fittizio->getNodo()))
    {
        ...
        for (int i=0; i<100; i++) interfaccia.scrivi_carattere(orizz);
        if (arrivo.getNodo()==fittizio->getNodo())
            interfaccia.scrivi("Non hai definito il luogo di arrivo, impossibile calcolare il percorso ...");
        interfaccia.a_capo();
        if (arrivo.getNodo()==partenza.getNodo())
            interfaccia.scrivi("Il luogo di partenza coincide con quello di arrivo, non e' necessario continuare !");
        interfaccia.a_capo();
        bool problemi=(partenza.getNodo()==fittizio->getNodo()
                       || arrivo.getNodo()==fittizio->getNodo() || arrivo.getNodo()==partenza.getNodo());
    // (fine integrazione Spina)


        if (!problemi) //se non si � verificata una delle condizioni precedenti
        {
            //controllo prima se ho il cammino sul file
            ifstream f("Cammini.nav",ios::in);
            if (f.good())
            {
                Coda< Grafo<etichetta,peso>::nodo > cammino; //coda che conterr� la sequenza del cammino
                string buffer; //conterr� le righe del file
                ...
                while (!f.eof())
                {
                    ...
                    sorg=nodo_da_id(id_s); //prendo il nodo il cui id nell'etichetta � id_s
                    dest=nodo_da_id(id_d); //prendo il nodo il cui id nell'etichetta � id_d
                    ...
                    {
                        //ho gi� il cammino calcolato, quindi lo inserisco nella coda
                        while (inizio<buffer.length())
                        {
                            id_nodo=atoi(estrai(buffer,inizio,',').c_str()); //prendo l'id del nodo
                            node=nodo_da_id(id_nodo); //prendo il nodo il cui id nell'etichetta � id_node
                ...
                {
                    ofstream f("Cammini.nav",ios::app); //apro il file in modalit� scrittura (salver� il cammino che calcoler�)
                    Pila< Grafo<etichetta,peso>::nodo > cammino; //pila che conterr� la sequenza del cammino che sar� calcolato
                    nodo_prec* T;
...
bool Mappa::multi_string_matching(string testo,string pattern)
{
    ...
    } // l'esito � positivo se e sole se ogni parte del pattern ha corrispondenza nel testo
    return (trovato);
}

Mappa::nodo_prec* Mappa::Dijkstra(Grafo<etichetta,peso>::nodo r) //Dijkstra radicato in r
{
    nodo_prec* T=new nodo_prec[numnodi]; //vettore di predecessori che sar� restituito
    float costi[numnodi]; //vettore dei costi del cammino da r al nodo i-esimo
    float costo; //costo che sar� tempo o lunghezza
    ...
    Lista< Grafo<etichetta,peso>::nodo > adiac; //avr� gli adiacenti di ogni nodo ad ogni passo
    temp.scrivinodo(r); //        ------+
    ...
    while (k<numnodi)//-------------------------------------------------------------+
    {
        ...
        // (inizio integrazione Spina)
        if (i.getNodo()!=r.getNodo()) { // se il nodo corrente non � r
        // (fine integrazione Spina)
            ...
            T[k]=temp;    //                                                        |
            costi[k]=999999; // la distanza da r � infinito (non sono collegati)    |
            k++; //                                                                 |
            l++;   //                                                               |
        }   //                                                                      |
        else
    ...
    while (!S.insiemevuoto()) //mentre non ho esaminato tutti i nodi del grafo
    {
        ...
        S.cancellamin(); //elimino il minimo dalla coda con priorit�
        ...
        while(!adiac.finelista(p)) //per tutti gli adiacenti del nodo
        {
            ...
            {
                // (inizio integrazione Spina)
                if (T[k].legginodo().getNodo()==j.getNodo()) {
                // (fine integrazione Spina)
                    trovato=true;
                    cursore_j=k;
                }
            ...
            if (!problemi) //se l'arco letto rispetta le opzioni desiderate
            {
                if (costi[cursore_i]+costo<costi[cursore_j]) //controllo se conservo la sottostruttura ottima
                {
                    ...
                    if (!S.appartiene(e)) //se il nodo (adiacente a quello che sto esaminando) non � gi� in coda
                    {
...
Pila< Grafo<Mappa::etichetta,Mappa::peso>::nodo > Mappa::cammino_da_dijkstra(nodo_prec* T,Grafo<etichetta,peso>::nodo d)
{
    ...
    Pila< Grafo<etichetta,peso>::nodo > P; //uso la pila perch� il cammino lo memorizzo dalla destinazione alla sorgente usando l'accesso LIFO viene "ordinato")
    int k=0;
    bool trovato=false;
    while (k<numnodi && !trovato) //ricerco d nel vettore
    {
        // (inizio integrazione Spina)                                                                       |
        if (T[k].legginodo().getNodo()==d.getNodo())  trovato=true;
        else k++;
    }
    ...
    {
        svuotaPila(P);//svuota la pila perch� non esiste un cammino dalla radice a d
    }
    return(P); //restituisco la pila
}

void Mappa::indicazioni(Coda< Grafo<etichetta,peso>::nodo > C) //stampa a video del cammino (questo metodo � richiamato quando ho gi� il cammino sul file)
{
    ...
    while(!C.codavuota()) //finch� ci sono altri nodi nella sequenza
    {
        ...
        tempo_totale+=tempo;
        via=mappa->leggiarco(s,d).legginome(); //leggo il nome della via dal peso dell'arco che va da sorgente a destinazione
        //gli IF che seguono sono eslcusivi per cui non c'� bisogno di annidarli
        if (vecchia==nuova) //se lo spostamento di prima e il corrente hanno stesso verso e direzione
        {
            //allora bisogna proseguire diritto/lungo la strada e non stampo il nome della via perch� � sempre la stessa)
            interfaccia.scrivi_parziale("Da ");
            interfaccia.scrivi_parziale(sorg);
        ...
        {
            //se il nuovo spostamento � a destra rispetto al vecchio bisogna svoltare a destra
            interfaccia.scrivi_parziale("Da ");
            interfaccia.scrivi_parziale(sorg);
        ...
        {
            //se il nuovo spostamento � a destra rispetto al vecchio bisogna svoltare a sinistra
            interfaccia.scrivi_parziale("Da ");
            interfaccia.scrivi_parziale(sorg);
...
{
    //analogo al precedente, ma viene richiamato quando il cammino non esiste su file ed � calcolato (in pi� c'� il salvataggio del cammino sul file)
    ...
    while(!P.pilavuota()) //finch� ci sono altri nodi nella sequenza
    {
        s=d; //la vecchia destinazione diventa sorgente
        sorg=dest;
...
void Mappa::cambia_opzioni()//permette di cambiare le opzioni su cui si baser� il calcolo del cammino migliore tramite un menu
{
    char orizz=205; //carattere per la stampa delle cornici
    int scelta;
    do
    {
        CLEARSCREEN(); //Modifica Samuele Pesce per porting universale //Modificato da ANTONIO PASTORELLI
        fflush(stdin);
...
void Mappa::setsuggerimenti(ifstream& file) //costruisce l'albero dei suggerimenti da file
{
    ...
    if(!file.eof())
    {
        do
        {
            ...
            if(b[0] >= 'A' && b[0] <= 'Z') //se il primo carattere � una lettera maiuscola allora � un luogo o un indirizzo e quindi viene inserito
            {
                do //scandisce l'array e inserisce lettera per lettera il luogo o l'indirizzo nell'albero
                {
                    ...
                    else //... altrimenti lo inserisco come fratello dei figli gi� presenti, a meno che quella lettera non � gi� presente tra i figli
                    {
        ...
        while(!file.eof());//... finch� non finisce il file
    }
}
...
string Mappa::getsuggerimenti(string input,string Partarr) //ritorna la stringa presa in input o scelta tra i suggerimenti messi a disposizione e ne stampa a video i risultati in tempo reale
{
    ...
    ins = new Lista<char*>; //lista che conterr� tutti i suggerimenti possibili a partire dalla parola scritta in input
    Lista<char*>::posizione pos = ins->primolista();
    while(ch != '\n') //se il carattere digitato � diverso dall'invio allora incomincio la ricerca
    {
        ...
        {
            //                                            |
            app.assign(input,0,input.length()-1); //    +-\ se il carattere digitato � uguale al backspace e se precedentemente � gi�
        ...
        {
            //                                            |
            if(ch != 63 && ch != 42 && ch != 38) //     |   se il carattere digitato � diverso dal backspace
            {
                //                                        +-> e se � diverso da '*', '?' e '&' (caratteri speciali che spiegher� successivamente)
                input.append(1,ch); //                  |   allora inserisco il carattere preso in input nella stringa
            } //                                        |
        } //--------------------------------------------+
        char* parola = new char[100];
        length = input.copy(parola,100,0); //copio il contenuto della stringa in un array di caratteri di appoggio che mi servir� per la ricerca
        parola[length] = '\0';
        ...
        if(ch == 38) //--------------------------------+
        {
            ...
            {
                //                                       |
                settapartenzacrono(); //               |   se il carattere digitato � uguale a '&' allora inserisco la partenza
            ...
            {
                //                                       |   da cui questa � stata richiamata che l'inserimento � avenuto tramite cronologia
                settaarrivocrono(); //                 |
            } //                                       |
            return "&"; //                             |
        } //-------------------------------------------+
        else if(ch != 63 || (ch == 63 && length == 0)) //se il carattere che � stato digitato � diverso da '?', oppure se � uguale a '?' e precedentemente non � stato digitato alcun carattere, allora procedo con i suggerimenti dei luoghi e degli indirizzi completi
        {
            for(int j=0; j<100; j++)
            {
                interfaccia.scrivi_parziale("_");
            ...
            if(length >= 1) //-----------------------------------+
            {
                //                                                 |-\ se si � digitato almeno un carattere allora procedo con la ricerca di quel nodo nell'albero
            ...
            int j = i;
            if(j != 0 && length == 1) //se ho trovato almeno un nodo e se si � digitato un solo carattere allora...
            {
                do
                {
                    j--;
                    ...
                    while(ritornafrase(parola2,nodi[j])) //... e mi salvo tutto il sottoalbero di ciascun nodo in un array che ad ogni ciclo inizializzo ogni volta
                    {
                        ...
                        {
                            //                                     |   se il nodo di partenza non � figlio della radice allora
                        ...
                        parola1[l] = '\0';
                        strcat(parola1,parola2); //appendo all'ultimo array utilizzato il contenuto dell'array dove � memorizzata una stringa presa dal sottoalbero del nodo di partenza
                ...
                while(j > 0); //finch� l'array dei nodi che ho trovato non termina
            }
            else if(length > 1 && ch != 63 && i != 0) //se sono stati digitati pi� caratteri l'ultimo � diverso da '?' ed � stato trovato almeno un nodo allora...
            {
                do //finch� non controllo tutti i nodi trovati (rigo 898)...
                {
                    j--;
                    nalbero<char>::nodo tempo = nodi[j];
                    int l = 1;
                    bool ok = true;
                    while(ok && parola[l] != '\0') //... finch� dalla parola digitata si ottengono dei suggerimenti...
                    {
                        ...
                        {
                            //                                                                                                                                            |-\ ... finch� uno dei figli non contiene il carattere presente nella stringa digitata
                            tempo = na.succnodo(tempo); //                                                                                                              |-/ e finch� ci sono altri fratelli, allora vado al fratello successivo...
                        } //--------------------------------------------------------------------------------------------------------------------------------------------+
            ...
            if(ch != 42 || (ch == 42 && length == 0)) //------------+
            {
                ...
                if(length >= 1 && !ins->listavuota()) //            |
                {
                    ...
                    do //                                           |
                    {
                        ...
                        pos = ins->succlista(pos); //               |   1) se l'ultimo carattere digitato � diverso da '*' o se � uguale ad esso ma non �
            ...
            else // 2)altrimenti procedo come segue:
            {
                do
                {
                    ...
                    else //-------------------------------------------------------------+
                    {
                        ...
                        interfaccia.scrivi_parziale(" ");//                             +-/ finch� non � nel range di quelle possibili
                        //system("pause"); //                                             |
                    } //----------------------------------------------------------------+
                }
                while(scelta > i);
        ...
        ins = new Lista<char*>; //da qu� in poi il procedimento si ripete per le singole parole, seguendo lo stesso ragionamento fatto fin'ora
        pos = ins->primolista(); //commenter� solo quelle righe di codice che si differenziano da quelle precedenti
        parola = new char[100];
        length = input.length();
        ...
        if(length >= 1 && ch != ' ' && j != 0)
        {
            do
            {
                ...
                if(parola[l] == '\0' && ok)
                {
                    ...
                    input.copy(parola2,100,l+1); //... dopodich� mi salvo l'ultima parola digitata in un array di appoggio
                    ...
                    while(ritornaparola(parola2,tempo)) //------+
                    {
                        ...
                        input.copy(parola2,100,l+1); //         |   trovo uno spazio o finch� non arrivo ad un nodo foglia
                        parola2[input.length()-l-1] = '\0'; //  |
                    } //----------------------------------------+
        ...
        pos = ins->primolista(); //ora procedo con la stampa: anche qu� ci sono due diversi tipi di stampe:
        ...
        else //-----------------------------------------+
        {
            ...
            ch = input[input.length()-1]; //            +-\ che � quella dove salvo normalmente l'input da tastiera, e lo elimino
...
void Mappa::setcronologia(fstream& file) //carica la cronologia salvata in un file
{
    ...
    if(!file.eof())
    {
        ...
        while(b[0] != ',') //finch� � diversa da ',', separatore tra cronologia delle partenze e cronologia degli arrivi...
        {
        ...
        {
            //                            |
            arrivi.inpila(b); //        +-> finch� non arrivo alla fine del file faccio la stessa cosa per le destinazioni
            getline(file,b); //         |
        } //----------------------------+
    }
}
...
void Mappa::salvacronologia(fstream& file) //salva la cronologia delle partenze e degli arrivi su file
{
    ...
    svuotaPila(partenze);
    file<<','<<endl; //quando la pila delle partenze � vuota, inserisco una virgola
...
{
    Pila<string> temp; //mi creo una pila di appoggio che mi servir� pi� avanti
    int i = 0;
    if(luogo.compare("partenza") == 0) //se devo stampare la cronologia delle partenze allora...
    {
        while(!partenze.pilavuota()) //... finch� la pila non � vuota...
        {
            interfaccia.scrivi_parziale(" ");
            interfaccia.scrivi_parziale(i+1);
            interfaccia.scrivi_parziale(") ");
        ...
        while(!temp.pilavuota()) //una volta stampato tutto il contenuto...
        {
            ...
            temp.fuoripila(); //... grazie alla pila d'appoggio che mi sono creato e questa la svuoto perch� ormai non mi serve pi�
        }
    }
    else //---------------------------------------------------------+
    {
        ...
        if(scelta >= 0 && scelta <= i)
        {
            ...
            else
            {
                ...
                while(!partenze.pilavuota() && j < scelta - 1) //-------+
                {
                    ...
                    partenze.fuoripila(); //                            +-/ perdere gli eventuali altri elementi della cronologia pi� recenti
                    j++; //                                             |
                } //----------------------------------------------------+
                part = partenze.leggipila(); //dopodich� salvo in contenuto in una stringa
                temp1 = nodo_da_input(part); //se non ho inserito numero civico cerco nei luoghi e nelle vie

               // (inizio integrazione Spina)
                if (temp1.getNodo() != fittizio->getNodo()) {
               // (fine integrazione Spina)
...
bool Mappa::ritornafrase(char* par,nalbero<char>::nodo nod) //ritorna true se � stata trovato un percorso non visitato dal nodo dato in input a una foglia, false altrimenti
{
    int cont = 0;
    while(par[cont] != '\0') //-----+
    {
        //                            +-\ finch� non arriva a fine parola
        cont++; //                  +-/ incrementa il contatore
    } //----------------------------+
    nalbero<char>::nodo temp = nod;
    while(!na.leggivisita(temp)) //finch� non trova un nodo gi� visitato...
    {
        if(!na.foglia(temp)) //... se il nodo non � una foglia...
        {
            temp = na.primofiglio(temp);//... allora vai a primo figlio...
            while(na.leggivisita(temp) && !na.finefratelli(temp) && na.succnodo(temp) != temp) //---+
            {
                //                                                                                    +-\ ... se temp � gi� stato visitato ed ha fratelli allora vado ai fratelli successivi
    ...
    if(na.foglia(temp)) //a questo punto, se sono arrivato ad una foglia, procedo a settare come visitato il primo antenato che ha un unico percorso che va' da quel nodo alla foglia temp
    {
        ...
        {
            //                                                                                                                                                        +-\ finch� il padre � figlio unico o � gi� stato
            temp1 = na.padre(temp1); //                                                                                                                             +-/ visitato o non � la radice, vado al padre
        } //--------------------------------------------------------------------------------------------------------------------------------------------------------+
...
bool Mappa::ritornaparola(char* par,nalbero<char>::nodo nod) //ritorna true se � stata trovato un percorso non visitato dal nodo dato in input a una foglia o ad un nodo che contiene uno spazio, false altrimenti
{
    int cont = 0;
    while(par[cont] != '\0') //-----+
    {
    ...
    while(!na.leggivisita(temp)) //finch� non trova un nodo gi� visitato...
    {
        if(!na.foglia(temp) && na.legginodo(temp) != ' ') //... se il nodo non � una foglia e l'elemento che contiene non � uno spazio...
        {
            temp = na.primofiglio(temp);//... allora vai a primo figlio...
            while(na.leggivisita(temp) && !na.finefratelli(temp) && na.succnodo(temp) != temp) //---+
            {
            ...
            if(na.legginodo(temp) != ' ') //... se l'elemento non � uguale a uno spazio
            {
...
void Mappa::ricercanodo(int& i,nalbero<char>::nodo* nodi,char e,nalbero<char>::nodo nod) //cerca all'interno dell'albero se � presente un nodo avente l'elemento passato in input
{
    if(!na.nalberovuoto()) //se 'albero non � vuoto procedo con la visita simmetrica...
    {
        if(!na.foglia(nod))
        {
...
void Mappa::epurazione() //elimina eventuali parole presenti pi� di una volta all'interno della lista di quelle trovate nell'albero durante la ricerca dei suggerimenti
{
    if(!ins->listavuota())
    {
...
void Mappa::inssottoalbero(char e,nalbero<char>::nodo& nodp,nalbero<char>::nodo& nodf) //inserisce il nodo la cui posizione sar� identificata da due nodi
{
    if(nodp == nodf) //se i due puntatori puntano alla stessa cella allora...
    {
        ...
        na.insarcofratello(fig,fig); //... setto definitivamente i fratelli (che in questo caso corrispondono a se stesso perch� � figlio unico)...
    ...
    else if(nodp != nodf && controllo(e,nodf)) //... altrimenti se nodp punta al padre e nodf punta al figlio e se l'elemento da inserire non � gi� presente tra i figli di nodp...
    {
        nalbero<char>::nodo fig = na.nuovonodo(); //creo una nuova cella
        ...
        na.insarcofratello(nodf,fig); //... setto definitivamente i fratelli (che in questo caso avverr� in ordine alfabetico)...
        na.settafiglio(fig); //---------+
...
bool Mappa::controllo(char elem,nalbero<char>::nodo& nod) //ritorna true se tra i fratelli del nodo passato in input c'� lo stesso elemento passato anch'esso in input, false altrimenti
{
...
Luogo Mappa::get_luogo(int id)
{
    ...
    // (id � 1) perch� il vettore �riferimenti[]� che contiene le coppie nodo_etichetta parte da
...
// (inizio integrazione Spina)
float Mappa::get_lunghezza(int p, int s){   //restituisce la lunghezza dell'arco passando l'id del nodo di partenza e di arrivo

    return mappa->leggiarco(nodo_da_id(p),nodo_da_id(s)).leggilunghezza();
}
// (fine integrazione Spina)

// inizio modifiche Davide Rano
Grafo<Mappa::etichetta,Mappa::peso>::nodo Mappa::nodo_da_nome_etichetta(string pattern) //restituisce un nodo ricevendo in input (parte del) l'etichetta
{
    Grafo<etichetta,peso>::nodo rif=*fittizio;
    int i=0; //----------+
    string testo; //     +-- comodo
    ...
    return(risultati.leggilista(risultati.primolista()).legginodo());
}
