...
{
    bool trovato = false;
    Oggetti inventario = oggetti.posizionati_in(0); // RISOLUZIONE BUG PIERLUIGI BEMPORTATO
    for(int i = 1; i <= inventario.get_n_oggetti(); i++)
    {

        if(inventario.get_oggetto(i).get_codice() == 34)//RISOLUZIONE BUG PIERLUIGI BEMPORTATO
        {
            trovato = true;
        }
    }
